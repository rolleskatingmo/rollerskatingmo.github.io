...
if (isMakeupMode) {
    ...
    async function markMakeupAttendance(name) {
        ...
        try {
            const data = await fetchWithFallback('markAttendanceMakeup', { name, operator: currentUser.username, date: makeupDate });
            if (data.success) {
...
function updateAttendanceList() {
    ...
    function toggleAttendanceStudent(name) {
        if (window.selectedAttendanceStudents.has(name)) {
            window.selectedAttendanceStudents.delete(name);
        } else {
            window.selectedAttendanceStudents.add(name);
        }
        updateAttendanceList();
    }
